...
def MY_MODEL(img_height, img_width, nclasses=19):
    ...
    base_model = MobileNetV2(input_tensor=shared_input, weights='imagenet', include_top=False)
    
    image_features = base_model.get_layer('block_16_project_BN').output
    
    x_a = ASPP(image_features)
    x_a = Upsample(tensor=x_a, size=[img_height // 4, img_width // 4])

    x_b = base_model.get_layer('expanded_conv_depthwise_relu').output
    ...
    feature_layer = base_model.get_layer('block_6_depthwise_relu').output
