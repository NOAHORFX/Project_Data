...
from tensorflow.keras.layers import AveragePooling2D, Lambda, Conv2D, Conv2DTranspose, Activation, Reshape, concatenate, Concatenate, BatchNormalization, ZeroPadding2D, Input, Add, ReLU, GlobalAveragePooling2D, Multiply, Softmax, multiply
...
def MY_MODEL(img_height, img_width, nclasses=19):
    ...
    base_model = Xception(input_tensor=shared_input, weights='imagenet', include_top=False)
    ...
    x_b = base_model.get_layer('block1_conv1_act').output
    ...
    feature_layer = base_model.get_layer('block6_sepconv2_act').output
